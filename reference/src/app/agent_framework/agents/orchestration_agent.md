# Orchestration Agent - Orchestration Agent Factory and System Prompt

## Component Overview

Orchestration Agent factory module that creates AWS infrastructure management agents with
...
- `OrchestrationAgent::create_system_prompt()` - Returns AI agent system prompt
- `OrchestrationAgent::create()` - Async factory method creating configured Agent
...
Pseudocode:
 ...
 11. Log BridgeDebugEvent::OrchestrationAgentStart for debugging
...
### UI Integration (agent_control_window.rs)
UI calls `OrchestrationAgent::create()` with user credentials and model selection
...
### Unit Tests
- `test_orchestration_agent_creation()` - Verifies successful agent instantiation
...
**Source File**: `src/app/agent_framework/agents/orchestration_agent.rs`
**Related Files**:
  - `src/app/agent_framework/callback_handlers.rs` - Event handling
  - `src/app/agent_framework/agents/task_agent.rs` - Subtask execution
  - `src/app/dashui/agent_control_window.rs` - UI integration
