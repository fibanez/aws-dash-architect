...
impl DataSyncService {
    ...
    pub async fn list_tasks(
        ...
        loop {
            ...
            match request.send().await {
                ...
                Err(e) => {
                    log::warn!(
                        "Failed to list DataSync tasks in account {} region {}: {}",
                        account_id,
                        region,
                        e
                    );
    ...
    pub async fn describe_task(
        ...
        let client = datasync::Client::new(&aws_config);
        let response = client.describe_task().task_arn(task_arn).send().await?;
    ...
    pub async fn list_locations(
        ...
        loop {
            ...
            match request.send().await {
                ...
                Err(e) => {
                    log::warn!(
                        "Failed to list DataSync locations in account {} region {}: {}",
                        account_id,
                        region,
                        e
                    );
    ...
    fn task_to_json(&self, task: &datasync::types::TaskListEntry) -> serde_json::Value {
        ...
        json.insert(
            ...
            serde_json::Value::String("AWS::DataSync::Task".to_string()),
        );

        serde_json::Value::Object(json)
    }

    fn task_details_to_json(
        &self,
        response: &datasync::operation::describe_task::DescribeTaskOutput,
    ) -> serde_json::Value {
        let mut json = serde_json::Map::new();
        ...
        if let Some(source_location_arn) = &response.source_location_arn {
            json.insert(
                ...
                serde_json::Value::String(source_location_arn.clone()),
            );
        }
        ...
        if let Some(destination_location_arn) = &response.destination_location_arn {
            json.insert(
                ...
                serde_json::Value::String(destination_location_arn.clone()),
            );
        }
        ...
        if let Some(cloud_watch_log_group_arn) = &response.cloud_watch_log_group_arn {
            json.insert(
                ...
                serde_json::Value::String(cloud_watch_log_group_arn.clone()),
            );
        }
        ...
        if let Some(options) = &response.options {
            ...
            if let Some(verify_mode) = &options.verify_mode {
                options_json.insert(
                    ...
                    serde_json::Value::String(verify_mode.as_str().to_string()),
                );
            }
            ...
            if let Some(overwrite_mode) = &options.overwrite_mode {
                options_json.insert(
                    ...
                    serde_json::Value::String(overwrite_mode.as_str().to_string()),
                );
            }
            ...
            if let Some(atime) = &options.atime {
                options_json.insert(
                    "Atime".to_string(),
                    serde_json::Value::String(atime.as_str().to_string()),
                );
            }
            ...
            if let Some(mtime) = &options.mtime {
                options_json.insert(
                    "Mtime".to_string(),
                    serde_json::Value::String(mtime.as_str().to_string()),
                );
            }
            ...
            if let Some(gid) = &options.gid {
                options_json.insert(
                    "Gid".to_string(),
                    serde_json::Value::String(gid.as_str().to_string()),
                );
            }
        ...
        if let Some(excludes) = &response.excludes {
                ...
                .map(|exclude| {
                    ...
                    if let Some(filter_type) = &exclude.filter_type {
                        exclude_json.insert(
                            ...
                            serde_json::Value::String(filter_type.as_str().to_string()),
                        );
                    }
            ...
            json.insert(
                "Excludes".to_string(),
                serde_json::Value::Array(excludes_array),
            );
        }

        if let Some(schedule) = &response.schedule {
            let mut schedule_json = serde_json::Map::new();
            schedule_json.insert(
                "ScheduleExpression".to_string(),
                serde_json::Value::String(schedule.schedule_expression.clone()),
            );
            json.insert(
                "Schedule".to_string(),
                serde_json::Value::Object(schedule_json),
            );
        }
        ...
        if let Some(error_detail) = &response.error_detail {
            json.insert(
                ...
                serde_json::Value::String(error_detail.clone()),
            );
        }
    ...
    fn location_to_json(&self, location: &datasync::types::LocationListEntry) -> serde_json::Value {
        ...
        if let Some(location_arn) = &location.location_arn {
            json.insert(
                "LocationArn".to_string(),
                serde_json::Value::String(location_arn.clone()),
            );
            json.insert(
                "ResourceId".to_string(),
                serde_json::Value::String(location_arn.clone()),
        ...
        json.insert(
            "ResourceType".to_string(),
            serde_json::Value::String("AWS::DataSync::Location".to_string()),
        );
