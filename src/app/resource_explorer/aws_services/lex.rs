...
impl LexService {
    ...
    pub async fn list_bots(
        ...
        let mut paginator = client.list_bots().into_paginator().send();
    ...
    async fn describe_bot_internal(
        ...
        let response = client.describe_bot().bot_id(bot_id).send().await?;

        let mut bot_details = self.bot_summary_to_json_from_describe(&response);

        // Get bot versions for additional details
        if let Ok(versions_response) = client.list_bot_versions().bot_id(bot_id).send().await {
                    ...
                    .map(|version| {
                        ...
                        if let Some(bot_version) = &version.bot_version {
                            version_json.insert(
                                ...
                                serde_json::Value::String(bot_version.clone()),
                            );
                        }
                        if let Some(bot_status) = &version.bot_status {
                            version_json.insert(
                                "BotStatus".to_string(),
                                serde_json::Value::String(bot_status.as_str().to_string()),
                            );
                        }
    ...
    fn bot_summary_to_json(&self, bot_summary: &lex::types::BotSummary) -> serde_json::Value {
        ...
        if let Some(bot_name) = &bot_summary.bot_name {
            json.insert(
                "BotName".to_string(),
                serde_json::Value::String(bot_name.clone()),
            );
            json.insert(
                "Name".to_string(),
                serde_json::Value::String(bot_name.clone()),
        ...
        if let Some(latest_bot_version) = &bot_summary.latest_bot_version {
            json.insert(
                "LatestBotVersion".to_string(),
                serde_json::Value::String(latest_bot_version.clone()),
            );
        }

        if let Some(last_updated_date_time) = bot_summary.last_updated_date_time {
            json.insert(
                "LastUpdatedDateTime".to_string(),
                serde_json::Value::String(last_updated_date_time.to_string()),
            );
        }
        ...
        if let Some(bot_type) = &bot_summary.bot_type {
            json.insert(
                "BotType".to_string(),
                serde_json::Value::String(bot_type.as_str().to_string()),
            );
        }
    ...
    fn bot_summary_to_json_from_describe(
        &self,
        response: &lex::operation::describe_bot::DescribeBotOutput,
    ) -> serde_json::Value {
        let mut json = serde_json::Map::new();
        ...
        if let Some(data_privacy) = &response.data_privacy {
            ...
            privacy_json.insert(
                ...
                serde_json::Value::Bool(data_privacy.child_directed),
            );
            json.insert(
                "DataPrivacy".to_string(),
                serde_json::Value::Object(privacy_json),
            );
        }
        ...
        if let Some(idle_session_ttl_in_seconds) = response.idle_session_ttl_in_seconds {
            json.insert(
                ...
                serde_json::Value::Number(serde_json::Number::from(idle_session_ttl_in_seconds)),
            );
        }
            ...
            if !bot_members.is_empty() {
                let members_json: Vec<serde_json::Value> = bot_members
                    ...
                    .map(|member| {
                        ...
                        member_json.insert(
                            "BotMemberId".to_string(),
                            serde_json::Value::String(member.bot_member_id.clone()),
                        );
                        member_json.insert(
                            "BotMemberName".to_string(),
                            serde_json::Value::String(member.bot_member_name.clone()),
                        );
                        member_json.insert(
                            "BotMemberAliasId".to_string(),
                            serde_json::Value::String(member.bot_member_alias_id.clone()),
                        );
                        member_json.insert(
                            "BotMemberAliasName".to_string(),
                            serde_json::Value::String(member.bot_member_alias_name.clone()),
                        );
                        member_json.insert(
                            "BotMemberVersion".to_string(),
                            serde_json::Value::String(member.bot_member_version.clone()),
                        );
                        serde_json::Value::Object(member_json)
                    })
                    .collect();
                json.insert(
                    "BotMembers".to_string(),
