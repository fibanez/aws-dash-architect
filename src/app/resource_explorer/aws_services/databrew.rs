...
impl DataBrewService {
    ...
    fn job_to_json(&self, job: &databrew::types::Job) -> serde_json::Value {
        ...
        json.insert("JobName".to_string(), serde_json::Value::String(job.name.clone()));
        json.insert("ResourceId".to_string(), serde_json::Value::String(job.name.clone()));
        json.insert("Name".to_string(), serde_json::Value::String(job.name.clone()));
        ...
        if let Some(create_date) = job.create_date {
            json.insert("CreateDate".to_string(), serde_json::Value::String(create_date.to_string()));
        }

        if let Some(last_modified_date) = job.last_modified_date {
            json.insert("LastModifiedDate".to_string(), serde_json::Value::String(last_modified_date.to_string()));
        }

        if let Some(last_modified_by) = &job.last_modified_by {
            json.insert("LastModifiedBy".to_string(), serde_json::Value::String(last_modified_by.clone()));
        }

        if let Some(job_type) = &job.r#type {
            json.insert("Type".to_string(), serde_json::Value::String(job_type.as_str().to_string()));
        }

        if let Some(project_name) = &job.project_name {
            json.insert("ProjectName".to_string(), serde_json::Value::String(project_name.clone()));
        }

        if let Some(dataset_name) = &job.dataset_name {
            json.insert("DatasetName".to_string(), serde_json::Value::String(dataset_name.clone()));
        }
        ...
        json.insert("Status".to_string(), serde_json::Value::String("UNKNOWN".to_string()));

        serde_json::Value::Object(json)
    }

    fn job_detail_to_json(&self, response: &databrew::operation::describe_job::DescribeJobOutput) -> serde_json::Value {
        let mut json = serde_json::Map::new();

        json.insert("JobName".to_string(), serde_json::Value::String(response.name.clone()));
        json.insert("ResourceId".to_string(), serde_json::Value::String(response.name.clone()));
        json.insert("Name".to_string(), serde_json::Value::String(response.name.clone()));
        ...
        if let Some(recipe_reference) = &response.recipe_reference {
            ...
            recipe_json.insert("Name".to_string(), serde_json::Value::String(recipe_reference.name.clone()));
            if let Some(recipe_version) = &recipe_reference.recipe_version {
                recipe_json.insert("RecipeVersion".to_string(), serde_json::Value::String(recipe_version.clone()));
            }
            json.insert("RecipeReference".to_string(), serde_json::Value::Object(recipe_json));
        }
        ...
        if let Some(encryption_key_arn) = &response.encryption_key_arn {
            json.insert("EncryptionKeyArn".to_string(), serde_json::Value::String(encryption_key_arn.clone()));
        }

        if let Some(encryption_mode) = &response.encryption_mode {
            json.insert("EncryptionMode".to_string(), serde_json::Value::String(encryption_mode.as_str().to_string()));
        }

        if let Some(log_subscription) = &response.log_subscription {
            json.insert("LogSubscription".to_string(), serde_json::Value::String(log_subscription.as_str().to_string()));
        }

        json.insert("MaxCapacity".to_string(), serde_json::Value::Number(serde_json::Number::from(response.max_capacity)));

        json.insert("MaxRetries".to_string(), serde_json::Value::Number(serde_json::Number::from(response.max_retries)));
            ...
            let outputs_json: Vec<serde_json::Value> = outputs
                ...
                .map(|output| {
                    ...
                    if let Some(location) = &output.location {
                        ...
                        location_json.insert("Bucket".to_string(), serde_json::Value::String(location.bucket.clone()));
                        if let Some(key) = &location.key {
                            location_json.insert("Key".to_string(), serde_json::Value::String(key.clone()));
                        }
                        output_json.insert("Location".to_string(), serde_json::Value::Object(location_json));
                    }
                    if let Some(compression_format) = &output.compression_format {
                        output_json.insert("CompressionFormat".to_string(), serde_json::Value::String(compression_format.as_str().to_string()));
                    }
                    if let Some(format) = &output.format {
                        output_json.insert("Format".to_string(), serde_json::Value::String(format.as_str().to_string()));
                    }
                    output_json.insert("Overwrite".to_string(), serde_json::Value::Bool(output.overwrite));
                    serde_json::Value::Object(output_json)
                })
                .collect();
            json.insert("Outputs".to_string(), serde_json::Value::Array(outputs_json));
        }
        ...
        json.insert("Timeout".to_string(), serde_json::Value::Number(serde_json::Number::from(response.timeout)));
        ...
        json.insert("Status".to_string(), serde_json::Value::String("READY".to_string()));
    ...
    fn dataset_to_json(&self, dataset: &databrew::types::Dataset) -> serde_json::Value {
        ...
        json.insert("DatasetName".to_string(), serde_json::Value::String(dataset.name.clone()));
        json.insert("ResourceId".to_string(), serde_json::Value::String(dataset.name.clone()));
        json.insert("Name".to_string(), serde_json::Value::String(dataset.name.clone()));
        ...
        if let Some(source) = &dataset.source {
            json.insert("Source".to_string(), serde_json::Value::String(source.as_str().to_string()));
        }
    ...
    fn dataset_detail_to_json(&self, response: &databrew::operation::describe_dataset::DescribeDatasetOutput) -> serde_json::Value {
        let mut json = serde_json::Map::new();

        json.insert("DatasetName".to_string(), serde_json::Value::String(response.name.clone()));
        json.insert("ResourceId".to_string(), serde_json::Value::String(response.name.clone()));
        json.insert("Name".to_string(), serde_json::Value::String(response.name.clone()));
        ...
        if let Some(_input) = &response.input {
            json.insert("Input".to_string(), serde_json::Value::String("TODO: Manual conversion needed".to_string()));
        }

        if let Some(_format_options) = &response.format_options {
            json.insert("FormatOptions".to_string(), serde_json::Value::String("TODO: Manual conversion needed".to_string()));
        }

        if let Some(_path_options) = &response.path_options {
            json.insert("PathOptions".to_string(), serde_json::Value::String("TODO: Manual conversion needed".to_string()));
        }
