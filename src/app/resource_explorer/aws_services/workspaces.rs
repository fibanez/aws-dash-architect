...
impl WorkSpacesService {
    ...
    pub async fn list_workspaces(
        ...
        loop {
            ...
            match request.send().await {
                ...
                Err(e) => {
                    log::warn!(
                        "Failed to list WorkSpaces in account {} region {}: {}",
                        account_id,
                        region,
                        e
                    );
    ...
    pub async fn list_directories(
        ...
        loop {
            ...
            match request.send().await {
                ...
                Err(e) => {
                    log::warn!(
                        "Failed to list WorkSpaces directories in account {} region {}: {}",
                        account_id,
                        region,
                        e
                    );
    ...
    fn workspace_to_json(&self, ws: &workspaces::types::Workspace) -> serde_json::Value {
        ...
        if let Some(computer_name) = &ws.computer_name {
            json.insert(
                ...
                serde_json::Value::String(computer_name.clone()),
            );
        }

        json.insert(
            "ResourceType".to_string(),
            serde_json::Value::String("AWS::WorkSpaces::Workspace".to_string()),
        );
        ...
        if let Some(json_obj) = base_json.as_object_mut() {
            ...
            if let Some(volume_encryption_key) = &ws.volume_encryption_key {
                json_obj.insert(
                    ...
                    serde_json::Value::String(volume_encryption_key.clone()),
                );
            }
            ...
            if let Some(user_volume_encryption_enabled) = &ws.user_volume_encryption_enabled {
                json_obj.insert(
                    ...
                    serde_json::Value::Bool(*user_volume_encryption_enabled),
                );
            }
            ...
            if let Some(root_volume_encryption_enabled) = &ws.root_volume_encryption_enabled {
                json_obj.insert(
                    ...
                    serde_json::Value::Bool(*root_volume_encryption_enabled),
                );
            }
            ...
            if let Some(workspace_properties) = &ws.workspace_properties {
                ...
                if let Some(running_mode) = &workspace_properties.running_mode {
                    properties_json.insert(
                        ...
                        serde_json::Value::String(running_mode.as_str().to_string()),
                    );
                }

                if let Some(running_mode_auto_stop_timeout_in_minutes) =
                ...
                if let Some(root_volume_size_gib) = &workspace_properties.root_volume_size_gib {
                    properties_json.insert(
                        ...
                        serde_json::Value::Number(serde_json::Number::from(*root_volume_size_gib)),
                    );
                }
                ...
                if let Some(user_volume_size_gib) = &workspace_properties.user_volume_size_gib {
                    properties_json.insert(
                        ...
                        serde_json::Value::Number(serde_json::Number::from(*user_volume_size_gib)),
                    );
                }
                ...
                if let Some(compute_type_name) = &workspace_properties.compute_type_name {
                    properties_json.insert(
                        ...
                        serde_json::Value::String(compute_type_name.as_str().to_string()),
                    );
                }

                json_obj.insert(
                    "WorkspaceProperties".to_string(),
                    serde_json::Value::Object(properties_json),
                );
            }
                ...
                let modifications_array: Vec<serde_json::Value> = modification_states
                    ...
                    .map(|modification| {
                        ...
                        if let Some(resource) = &modification.resource {
                            mod_json.insert(
                                "Resource".to_string(),
                                serde_json::Value::String(resource.as_str().to_string()),
                            );
                        }
                    ...
                    .collect();
                json_obj.insert(
                    "ModificationStates".to_string(),
                    serde_json::Value::Array(modifications_array),
                );
            }
        }
    ...
    fn directory_to_json(
        &self,
        directory: &workspaces::types::WorkspaceDirectory,
    ) -> serde_json::Value {
        let mut json = serde_json::Map::new();
        ...
        if let Some(directory_name) = &directory.directory_name {
            json.insert(
                ...
                serde_json::Value::String(directory_name.clone()),
            );
        }

        if let Some(registration_code) = &directory.registration_code {
            json.insert(
                "RegistrationCode".to_string(),
                serde_json::Value::String(registration_code.clone()),
            );
        }

        if let Some(directory_type) = &directory.directory_type {
            json.insert(
                "DirectoryType".to_string(),
                serde_json::Value::String(directory_type.as_str().to_string()),
            );
        }
        ...
        if let Some(subnet_ids) = &directory.subnet_ids {
            ...
            json.insert(
                "SubnetIds".to_string(),
                serde_json::Value::Array(subnets_array),
            );
        }
        ...
        if let Some(dns_ip_addresses) = &directory.dns_ip_addresses {
            ...
            json.insert(
                ...
                serde_json::Value::Array(dns_array),
            );
        }

        if let Some(customer_user_name) = &directory.customer_user_name {
            json.insert(
                "CustomerUserName".to_string(),
                serde_json::Value::String(customer_user_name.clone()),
            );
        }

        json.insert(
            "ResourceType".to_string(),
            serde_json::Value::String("AWS::WorkSpaces::Directory".to_string()),
        );
