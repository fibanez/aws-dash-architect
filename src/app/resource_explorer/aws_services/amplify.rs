...
impl AmplifyService {
    ...
    pub async fn list_apps(
        ...
        let mut paginator = client.list_apps().into_paginator().send();
    ...
    async fn get_app_internal(
        ...
        let response = client.get_app().app_id(app_id).send().await?;
    ...
    fn app_to_json(&self, app: &amplify::types::App) -> serde_json::Value {
        ...
        json.insert(
            "AppId".to_string(),
            serde_json::Value::String(app.app_id.clone()),
        );
        json.insert(
        ...
        json.insert(
            "AppArn".to_string(),
            serde_json::Value::String(app.app_arn.clone()),
        );
        json.insert(
            "Name".to_string(),
        ...
        json.insert(
            ...
            serde_json::Value::String(app.description.clone()),
        );
        json.insert(
            "Repository".to_string(),
            serde_json::Value::String(app.repository.clone()),
        );
        json.insert(
            "Platform".to_string(),
            serde_json::Value::String(app.platform.as_str().to_string()),
        );

        json.insert(
            "CreateTime".to_string(),
            serde_json::Value::String(app.create_time.to_string()),
        );
        json.insert(
            "UpdateTime".to_string(),
            serde_json::Value::String(app.update_time.to_string()),
        );

        if let Some(iam_service_role_arn) = &app.iam_service_role_arn {
            json.insert(
                "IamServiceRoleArn".to_string(),
                serde_json::Value::String(iam_service_role_arn.clone()),
            );
        }

        if !app.environment_variables.is_empty() {
            let env_vars_json: Vec<serde_json::Value> = app
            ...
            json.insert(
                ...
                serde_json::Value::Array(env_vars_json),
            );
        }

        json.insert(
            "DefaultDomain".to_string(),
            serde_json::Value::String(app.default_domain.clone()),
        );
        json.insert(
            "EnableBranchAutoBuild".to_string(),
            serde_json::Value::Bool(app.enable_branch_auto_build),
        );
        json.insert(
            "EnableBranchAutoDeletion".to_string(),
            serde_json::Value::Bool(app.enable_branch_auto_deletion.unwrap_or(false)),
        );
        json.insert(
            "EnableBasicAuth".to_string(),
            serde_json::Value::Bool(app.enable_basic_auth),
        );

        if let Some(basic_auth_credentials) = &app.basic_auth_credentials {
            json.insert(
                "BasicAuthCredentials".to_string(),
                serde_json::Value::String(basic_auth_credentials.clone()),
            );
        }
            ...
            if !custom_rules.is_empty() {
                let rules_json: Vec<serde_json::Value> = custom_rules
                    ...
                    .map(|rule| {
                        ...
                        rule_json.insert(
                            "Source".to_string(),
                            serde_json::Value::String(rule.source.clone()),
                        );
                        rule_json.insert(
                            "Target".to_string(),
        ...
        if let Some(production_branch) = &app.production_branch {
            ...
            if let Some(last_deploy_time) = production_branch.last_deploy_time {
                prod_branch_json.insert(
                    "LastDeployTime".to_string(),
                    serde_json::Value::String(last_deploy_time.to_string()),
                );
            }
            ...
            json.insert(
                "ProductionBranch".to_string(),
                serde_json::Value::Object(prod_branch_json),
            );
        }
        ...
        if let Some(build_spec) = &app.build_spec {
            json.insert(
                "BuildSpec".to_string(),
                serde_json::Value::String(build_spec.clone()),
            );
        }
        ...
        if let Some(custom_headers) = &app.custom_headers {
            json.insert(
                ...
                serde_json::Value::String(custom_headers.clone()),
            );
        }

        json.insert(
            "EnableAutoBranchCreation".to_string(),
            serde_json::Value::Bool(app.enable_auto_branch_creation.unwrap_or(false)),
        );
