...
impl TransferService {
    ...
    pub async fn list_servers(
        ...
        loop {
            ...
            match request.send().await {
                ...
                Err(e) => {
                    log::warn!("Failed to list Transfer Family servers in account {} region {}: {}", account_id, region, e);
                    break;
                }
            }
        }
    ...
    pub async fn describe_server(
        ...
        if let Some(server) = response.server {
            ...
            Err(anyhow::anyhow!("Transfer Family server {} not found", server_id))
        }
    }
    ...
    fn server_to_json(&self, server: &transfer::types::ListedServer) -> serde_json::Value {
        ...
        if let Some(server_id) = &server.server_id {
            json.insert("ServerId".to_string(), serde_json::Value::String(server_id.clone()));
            json.insert("ResourceId".to_string(), serde_json::Value::String(server_id.clone()));
        }

        json.insert("Arn".to_string(), serde_json::Value::String(server.arn.clone()));

        if let Some(domain) = &server.domain {
            json.insert("Domain".to_string(), serde_json::Value::String(domain.as_str().to_string()));
        }

        if let Some(identity_provider_type) = &server.identity_provider_type {
            json.insert("IdentityProviderType".to_string(), serde_json::Value::String(identity_provider_type.as_str().to_string()));
        }

        if let Some(endpoint_type) = &server.endpoint_type {
            json.insert("EndpointType".to_string(), serde_json::Value::String(endpoint_type.as_str().to_string()));
        }
        ...
        if let Some(user_count) = &server.user_count {
            json.insert("UserCount".to_string(), serde_json::Value::Number(serde_json::Number::from(*user_count)));
        }

        json.insert("ResourceType".to_string(), serde_json::Value::String("AWS::Transfer::Server".to_string()));

        serde_json::Value::Object(json)
    }

    fn server_details_to_json(&self, server: &transfer::types::DescribedServer) -> serde_json::Value {
        let mut json = serde_json::Map::new();
        ...
        if let Some(certificate) = &server.certificate {
            json.insert("Certificate".to_string(), serde_json::Value::String(certificate.clone()));
        }

        if let Some(logging_role) = &server.logging_role {
            json.insert("LoggingRole".to_string(), serde_json::Value::String(logging_role.clone()));
        }
        ...
        if let Some(protocols) = &server.protocols {
            let protocols_array: Vec<serde_json::Value> = protocols
                ...
                .collect();
            json.insert("Protocols".to_string(), serde_json::Value::Array(protocols_array));
        }

        // Security policy
        if let Some(security_policy_name) = &server.security_policy_name {
            json.insert("SecurityPolicyName".to_string(), serde_json::Value::String(security_policy_name.clone()));
        }

        // Host key fingerprint
        if let Some(host_key_fingerprint) = &server.host_key_fingerprint {
            json.insert("HostKeyFingerprint".to_string(), serde_json::Value::String(host_key_fingerprint.clone()));
        }
        ...
        if let Some(endpoint_details) = &server.endpoint_details {
            ...
            if let Some(address_allocation_ids) = &endpoint_details.address_allocation_ids {
                let ids_array: Vec<serde_json::Value> = address_allocation_ids
                    ...
                    .collect();
                endpoint_json.insert("AddressAllocationIds".to_string(), serde_json::Value::Array(ids_array));
            }
            ...
            if let Some(vpc_id) = &endpoint_details.vpc_id {
                endpoint_json.insert("VpcId".to_string(), serde_json::Value::String(vpc_id.clone()));
            }

            if let Some(vpc_endpoint_id) = &endpoint_details.vpc_endpoint_id {
                endpoint_json.insert("VpcEndpointId".to_string(), serde_json::Value::String(vpc_endpoint_id.clone()));
            }
            ...
            if let Some(security_group_ids) = &endpoint_details.security_group_ids {
                let ids_array: Vec<serde_json::Value> = security_group_ids
                    ...
                    .map(|id| serde_json::Value::String(id.clone()))
                    .collect();
                endpoint_json.insert("SecurityGroupIds".to_string(), serde_json::Value::Array(ids_array));
            }

            json.insert("EndpointDetails".to_string(), serde_json::Value::Object(endpoint_json));
        }
        ...
        if let Some(identity_provider_details) = &server.identity_provider_details {
            ...
            if let Some(invocation_role) = &identity_provider_details.invocation_role {
                idp_json.insert("InvocationRole".to_string(), serde_json::Value::String(invocation_role.clone()));
            }

            if let Some(directory_id) = &identity_provider_details.directory_id {
                idp_json.insert("DirectoryId".to_string(), serde_json::Value::String(directory_id.clone()));
            }

            json.insert("IdentityProviderDetails".to_string(), serde_json::Value::Object(idp_json));
        }
        ...
        if let Some(tags) = &server.tags {
            let tags_array: Vec<serde_json::Value> = tags
                ...
                .map(|tag| {
                    ...
                    tag_json.insert("Key".to_string(), serde_json::Value::String(tag.key.clone()));
                    tag_json.insert("Value".to_string(), serde_json::Value::String(tag.value.clone()));
    ...
    fn user_to_json(&self, user: &transfer::types::ListedUser, server_id: &str) -> serde_json::Value {
        let mut json = serde_json::Map::new();
        
        if let Some(username) = &user.user_name {
            json.insert("UserName".to_string(), serde_json::Value::String(username.clone()));
            json.insert("ResourceId".to_string(), serde_json::Value::String(format!("{}:{}", server_id, username)));
        }

        json.insert("ServerId".to_string(), serde_json::Value::String(server_id.to_string()));

        json.insert("Arn".to_string(), serde_json::Value::String(user.arn.clone()));

        if let Some(home_directory) = &user.home_directory {
            json.insert("HomeDirectory".to_string(), serde_json::Value::String(home_directory.clone()));
        }

        if let Some(home_directory_type) = &user.home_directory_type {
            json.insert("HomeDirectoryType".to_string(), serde_json::Value::String(home_directory_type.as_str().to_string()));
        }
        ...
        if let Some(ssh_public_key_count) = &user.ssh_public_key_count {
            json.insert("SshPublicKeyCount".to_string(), serde_json::Value::Number(serde_json::Number::from(*ssh_public_key_count)));
        }

        json.insert("ResourceType".to_string(), serde_json::Value::String("AWS::Transfer::User".to_string()));
