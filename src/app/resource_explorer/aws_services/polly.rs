...
impl PollyService {
    ...
    pub async fn list_speech_synthesis_tasks(
        ...
        let mut paginator = client.list_speech_synthesis_tasks().into_paginator().send();
    ...
    pub async fn get_speech_synthesis_task(
        ...
        let client = polly::Client::new(&aws_config);
        self.get_speech_synthesis_task_internal(&client, task_id)
            .await
    }
    ...
    async fn get_lexicon_internal(
        ...
        let response = client.get_lexicon().name(lexicon_name).send().await?;

        let mut json = serde_json::Map::new();

        json.insert(
            "Name".to_string(),
            serde_json::Value::String(lexicon_name.to_string()),
        );
        json.insert(
        ...
        if let Some(lexicon_attributes) = response.lexicon_attributes {
            ...
            if let Some(language_code) = lexicon_attributes.language_code {
                json.insert(
                    ...
                    serde_json::Value::String(language_code.as_str().to_string()),
                );
            }
            ...
            json.insert(
                "LexemesCount".to_string(),
                serde_json::Value::Number(serde_json::Number::from(
                    lexicon_attributes.lexemes_count,
                )),
            );
            json.insert(
                "Size".to_string(),
                serde_json::Value::Number(serde_json::Number::from(lexicon_attributes.size)),
            );
        }
    ...
    fn voice_to_json(&self, voice: &polly::types::Voice) -> serde_json::Value {
        ...
        if let Some(voice_id) = &voice.id {
            json.insert(
                "VoiceId".to_string(),
                serde_json::Value::String(voice_id.as_str().to_string()),
            );
            json.insert(
                "ResourceId".to_string(),
                serde_json::Value::String(voice_id.as_str().to_string()),
        ...
        if let Some(gender) = &voice.gender {
            json.insert(
                "Gender".to_string(),
                serde_json::Value::String(gender.as_str().to_string()),
            );
        }
        ...
        if let Some(language_name) = &voice.language_name {
            json.insert(
                "LanguageName".to_string(),
                serde_json::Value::String(language_name.clone()),
            );
        }
        ...
        if let Some(additional_language_codes) = &voice.additional_language_codes {
            if !additional_language_codes.is_empty() {
                let codes: Vec<serde_json::Value> = additional_language_codes
                    ...
                    .collect();
                json.insert(
                    "AdditionalLanguageCodes".to_string(),
        ...
        if let Some(supported_engines) = &voice.supported_engines {
            if !supported_engines.is_empty() {
                let engines: Vec<serde_json::Value> = supported_engines
                    ...
                    .collect();
                json.insert(
                    "SupportedEngines".to_string(),
                    serde_json::Value::Array(engines),
                );
            }
        }
    ...
    fn lexicon_to_json(&self, lexicon: &polly::types::LexiconDescription) -> serde_json::Value {
        ...
        if let Some(attributes) = &lexicon.attributes {
            if let Some(alphabet) = &attributes.alphabet {
                json.insert(
                    "Alphabet".to_string(),
                    serde_json::Value::String(alphabet.clone()),
                );
            }
            ...
            json.insert(
                "LexemesCount".to_string(),
                serde_json::Value::Number(serde_json::Number::from(attributes.lexemes_count)),
            );
            json.insert(
                "Size".to_string(),
                serde_json::Value::Number(serde_json::Number::from(attributes.size)),
            );
        }
    ...
    fn synthesis_task_to_json(&self, task: &polly::types::SynthesisTask) -> serde_json::Value {
        ...
        if let Some(task_status) = &task.task_status {
            json.insert(
                ...
                serde_json::Value::String(task_status.as_str().to_string()),
            );
            json.insert(
                "Status".to_string(),
                serde_json::Value::String(task_status.as_str().to_string()),
        ...
        if let Some(task_status_reason) = &task.task_status_reason {
            json.insert(
                "TaskStatusReason".to_string(),
                serde_json::Value::String(task_status_reason.clone()),
            );
        }
        ...
        if let Some(output_uri) = &task.output_uri {
            json.insert(
                "OutputUri".to_string(),
                serde_json::Value::String(output_uri.clone()),
            );
        }
        ...
        json.insert(
            "RequestCharacters".to_string(),
            serde_json::Value::Number(serde_json::Number::from(task.request_characters)),
        );
        ...
        if let Some(output_format) = &task.output_format {
            json.insert(
                ...
                serde_json::Value::String(output_format.as_str().to_string()),
            );
        }
        ...
        if let Some(sample_rate) = &task.sample_rate {
            json.insert(
                ...
                serde_json::Value::String(sample_rate.clone()),
            );
        }
        ...
        if let Some(text_type) = &task.text_type {
            json.insert(
                "TextType".to_string(),
                serde_json::Value::String(text_type.as_str().to_string()),
            );
        }
