...
impl GlobalAcceleratorService {
    ...
    pub async fn describe_accelerator(
        ...
        let client = globalaccelerator::Client::new(&aws_config);
        self.describe_accelerator_internal(&client, accelerator_arn).await
    }
    ...
    fn accelerator_to_json(&self, accelerator: &globalaccelerator::types::Accelerator) -> serde_json::Value {
        let mut json = serde_json::Map::new();
        
        if let Some(accelerator_arn) = &accelerator.accelerator_arn {
            json.insert("AcceleratorArn".to_string(), serde_json::Value::String(accelerator_arn.clone()));
            json.insert("ResourceId".to_string(), serde_json::Value::String(accelerator_arn.clone()));
        }
        ...
        json.insert("Enabled".to_string(), serde_json::Value::Bool(accelerator.enabled.unwrap_or(false)));

        if let Some(ip_address_type) = &accelerator.ip_address_type {
            json.insert("IpAddressType".to_string(), serde_json::Value::String(ip_address_type.as_str().to_string()));
        }
                ...
                let ip_sets_json: Vec<serde_json::Value> = ip_sets
                    ...
                    .map(|ip_set| {
                        ...
                        if let Some(ip_address_family) = &ip_set.ip_address_family {
                            ip_set_json.insert("IpAddressFamily".to_string(), serde_json::Value::String(ip_address_family.as_str().to_string()));
                        }
                        if let Some(ip_addresses) = &ip_set.ip_addresses {
                            if !ip_addresses.is_empty() {
                                let addresses: Vec<serde_json::Value> = ip_addresses
                                    ...
                                    .collect();
                                ip_set_json.insert("IpAddresses".to_string(), serde_json::Value::Array(addresses));
                            }
                        }
        ...
        if let Some(last_modified_time) = accelerator.last_modified_time {
            json.insert("LastModifiedTime".to_string(), serde_json::Value::String(last_modified_time.to_string()));
        }

        serde_json::Value::Object(json)
    }

    fn accelerator_details_to_json(&self, accelerator: &globalaccelerator::types::Accelerator) -> serde_json::Value {
