...
impl SecurityHubService {
    ...
    fn hub_to_json(&self, response: &securityhub::operation::describe_hub::DescribeHubOutput) -> serde_json::Value {
        let mut json = serde_json::Map::new();
        
        // Create a synthetic hub ARN for identification
        json.insert("HubArn".to_string(), serde_json::Value::String("SecurityHub".to_string()));
        json.insert("ResourceId".to_string(), serde_json::Value::String("SecurityHub".to_string()));
        json.insert("Name".to_string(), serde_json::Value::String("Security Hub".to_string()));

        if let Some(subscribed_at) = &response.subscribed_at {
            json.insert("SubscribedAt".to_string(), serde_json::Value::String(subscribed_at.to_string()));
        }

        json.insert("AutoEnableControls".to_string(), serde_json::Value::Bool(response.auto_enable_controls.unwrap_or(false)));

        if let Some(control_finding_generator) = &response.control_finding_generator {
            json.insert("ControlFindingGenerator".to_string(), serde_json::Value::String(control_finding_generator.as_str().to_string()));
        }
