...
impl MacieService {
    ...
    pub async fn list_classification_jobs(
        ...
        match client.get_macie_session().send().await {
            Ok(_response) => {
                ...
                json.insert("Name".to_string(), serde_json::Value::String("Macie Session".to_string()));
            ...
            Err(e) => {
                warn!("Macie not enabled or accessible for account {} in region {}: {}", account_id, region, e);
                // Create entry indicating Macie is not enabled
                let mut json = serde_json::Map::new();
                json.insert("AccountId".to_string(), serde_json::Value::String(account_id.to_string()));
                json.insert("ResourceId".to_string(), serde_json::Value::String(format!("macie-{}", account_id)));
                json.insert("Status".to_string(), serde_json::Value::String("Disabled".to_string()));
                json.insert("Name".to_string(), serde_json::Value::String("Macie (Disabled)".to_string()));
                resources.push(serde_json::Value::Object(json));
            }
        }
    ...
    pub async fn get_macie_session(
        ...
        json.insert("AccountId".to_string(), serde_json::Value::String(account_id.to_string()));
        json.insert("Status".to_string(), serde_json::Value::String("Enabled".to_string()));
        json.insert("Service".to_string(), serde_json::Value::String("Amazon Macie".to_string()));
        json.insert("Description".to_string(), serde_json::Value::String("Data security and data privacy service".to_string()));
