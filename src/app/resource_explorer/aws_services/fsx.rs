...
impl FsxService {
    ...
    pub async fn list_file_systems(
        ...
        loop {
            ...
            match request.send().await {
                ...
                Err(e) => {
                    log::warn!(
                        "Failed to list FSx file systems in account {} region {}: {}",
                        account_id,
                        region,
                        e
                    );
    ...
    pub async fn list_backups(
        ...
        loop {
            ...
            match request.send().await {
                ...
                Err(e) => {
                    log::warn!(
                        "Failed to list FSx backups in account {} region {}: {}",
                        account_id,
                        region,
                        e
                    );
    ...
    fn file_system_to_json(&self, fs: &fsx::types::FileSystem) -> serde_json::Value {
        ...
        if let Some(file_system_id) = &fs.file_system_id {
            json.insert(
                ...
                serde_json::Value::String(file_system_id.clone()),
            );
            json.insert(
        ...
        if let Some(file_system_type) = &fs.file_system_type {
            json.insert(
                ...
                serde_json::Value::String(file_system_type.as_str().to_string()),
            );
        }
        ...
        if let Some(lifecycle) = &fs.lifecycle {
            json.insert(
                "Lifecycle".to_string(),
                serde_json::Value::String(lifecycle.as_str().to_string()),
            );
        }
        ...
        if let Some(storage_capacity) = &fs.storage_capacity {
            json.insert(
                ...
                serde_json::Value::Number(serde_json::Number::from(*storage_capacity)),
            );
        }
        ...
        if let Some(storage_type) = &fs.storage_type {
            json.insert(
                ...
                serde_json::Value::String(storage_type.as_str().to_string()),
            );
        }
        ...
        json.insert(
            ...
            serde_json::Value::String("AWS::FSx::FileSystem".to_string()),
        );
        ...
        if let Some(json_obj) = base_json.as_object_mut() {
            ...
            if let Some(network_interface_ids) = &fs.network_interface_ids {
                ...
                json_obj.insert(
                    ...
                    serde_json::Value::Array(ni_array),
                );
            }
            ...
            if let Some(windows_config) = &fs.windows_configuration {
                ...
                if let Some(active_directory_id) = &windows_config.active_directory_id {
                    windows_json.insert(
                        ...
                        serde_json::Value::String(active_directory_id.clone()),
                    );
                }
                ...
                if let Some(throughput_capacity) = &windows_config.throughput_capacity {
                    windows_json.insert(
                        ...
                        serde_json::Value::Number(serde_json::Number::from(*throughput_capacity)),
                    );
                }

                if let Some(weekly_maintenance_start_time) =
                    &windows_config.weekly_maintenance_start_time
                {
                    windows_json.insert(
                        "WeeklyMaintenanceStartTime".to_string(),
                        serde_json::Value::String(weekly_maintenance_start_time.clone()),
                    );
                }

                json_obj.insert(
                    "WindowsConfiguration".to_string(),
                    serde_json::Value::Object(windows_json),
                );
            }
            ...
            if let Some(lustre_config) = &fs.lustre_configuration {
                ...
                if let Some(weekly_maintenance_start_time) =
                    &lustre_config.weekly_maintenance_start_time
                {
                    lustre_json.insert(
                        "WeeklyMaintenanceStartTime".to_string(),
                        serde_json::Value::String(weekly_maintenance_start_time.clone()),
                ...
                if let Some(data_repository_configuration) =
                    &lustre_config.data_repository_configuration
                {
                    let mut data_repo_json = serde_json::Map::new();

                    if let Some(lifecycle) = &data_repository_configuration.lifecycle {
                        data_repo_json.insert(
                            "Lifecycle".to_string(),
                            serde_json::Value::String(lifecycle.as_str().to_string()),
                        );
                    }
                    ...
                    if let Some(export_path) = &data_repository_configuration.export_path {
                        data_repo_json.insert(
                            ...
                            serde_json::Value::String(export_path.clone()),
                        );
                    }

                    lustre_json.insert(
                        "DataRepositoryConfiguration".to_string(),
                        serde_json::Value::Object(data_repo_json),
                    );
                }

                json_obj.insert(
                    "LustreConfiguration".to_string(),
                    serde_json::Value::Object(lustre_json),
                );
            }
    ...
    fn backup_to_json(&self, backup: &fsx::types::Backup) -> serde_json::Value {
        ...
        if let Some(backup_id) = &backup.backup_id {
            json.insert(
                "BackupId".to_string(),
                serde_json::Value::String(backup_id.clone()),
            );
            json.insert(
                "ResourceId".to_string(),
                serde_json::Value::String(backup_id.clone()),
        ...
        json.insert(
            "ResourceType".to_string(),
            serde_json::Value::String("AWS::FSx::Backup".to_string()),
        );
