...
impl ConnectService {
    ...
    async fn describe_instance_internal(
        ...
        if let Some(instance) = response.instance {
            ...
            Err(anyhow::anyhow!("Connect instance {} not found", instance_id))
        }
    }

    fn instance_summary_to_json(&self, instance_summary: &connect::types::InstanceSummary) -> serde_json::Value {
        ...
        if let Some(identity_management_type) = &instance_summary.identity_management_type {
            json.insert("IdentityManagementType".to_string(), serde_json::Value::String(identity_management_type.as_str().to_string()));
        }

        if let Some(instance_alias) = &instance_summary.instance_alias {
            json.insert("InstanceAlias".to_string(), serde_json::Value::String(instance_alias.clone()));
        }
        ...
        if let Some(service_role) = &instance_summary.service_role {
            json.insert("ServiceRole".to_string(), serde_json::Value::String(service_role.clone()));
        }

        if let Some(instance_status) = &instance_summary.instance_status {
            json.insert("InstanceStatus".to_string(), serde_json::Value::String(instance_status.as_str().to_string()));
        }

        json.insert("InboundCallsEnabled".to_string(), serde_json::Value::Bool(instance_summary.inbound_calls_enabled.unwrap_or(false)));
        json.insert("OutboundCallsEnabled".to_string(), serde_json::Value::Bool(instance_summary.outbound_calls_enabled.unwrap_or(false)));

        if let Some(instance_access_url) = &instance_summary.instance_access_url {
            json.insert("InstanceAccessUrl".to_string(), serde_json::Value::String(instance_access_url.clone()));
        }
    ...
    fn instance_to_json(&self, instance: &connect::types::Instance) -> serde_json::Value {
        ...
        if let Some(instance_status) = &instance.instance_status {
            json.insert("InstanceStatus".to_string(), serde_json::Value::String(instance_status.as_str().to_string()));
            json.insert("Status".to_string(), serde_json::Value::String(instance_status.as_str().to_string()));
        }

        json.insert("InboundCallsEnabled".to_string(), serde_json::Value::Bool(instance.inbound_calls_enabled.unwrap_or(false)));
        json.insert("OutboundCallsEnabled".to_string(), serde_json::Value::Bool(instance.outbound_calls_enabled.unwrap_or(false)));
        ...
        if let Some(status_reason) = &instance.status_reason {
            ...
            if let Some(message) = &status_reason.message {
                status_reason_json.insert("Message".to_string(), serde_json::Value::String(message.clone()));
            }
            json.insert("StatusReason".to_string(), serde_json::Value::Object(status_reason_json));
        }
