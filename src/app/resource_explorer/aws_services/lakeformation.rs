...
impl LakeFormationService {
    ...
    async fn describe_data_lake_settings_internal(
        ...
        let response = client.get_data_lake_settings().send().await?;

        Ok(self.data_lake_settings_to_json(&response))
    }

    fn data_lake_settings_to_json(
        &self,
        response: &lakeformation::operation::get_data_lake_settings::GetDataLakeSettingsOutput,
    ) -> serde_json::Value {
        ...
        json.insert(
            ...
            serde_json::Value::String("DataLakeSettings".to_string()),
        );
        json.insert(
            "Name".to_string(),
            serde_json::Value::String("Lake Formation Data Lake Settings".to_string()),
        );
        ...
        if let Some(data_lake_settings) = &response.data_lake_settings {
            if let Some(data_lake_admins) = &data_lake_settings.data_lake_admins {
                let admins_json: Vec<serde_json::Value> = data_lake_admins
                    ...
                    .map(|admin| {
                        ...
                        if let Some(data_lake_principal_identifier) =
                            &admin.data_lake_principal_identifier
                        {
                            admin_json.insert(
                                "DataLakePrincipalIdentifier".to_string(),
                                serde_json::Value::String(data_lake_principal_identifier.clone()),
                            );
                        }
                    ...
                    .collect();
                json.insert(
                    "DataLakeAdmins".to_string(),
                    serde_json::Value::Array(admins_json),
                );
            }

            if let Some(create_database_default_permissions) =
                &data_lake_settings.create_database_default_permissions
            {
                    ...
                    .map(|perm| {
                                ...
                                &principal.data_lake_principal_identifier
                            {
                                perm_json.insert(
                                    ...
                                    serde_json::Value::String(
                                        data_lake_principal_identifier.clone(),
                                    ),
                                );
                            }
                        }
                        if let Some(permissions) = &perm.permissions {
                            ...
                            perm_json.insert(
                                ...
                                serde_json::Value::Array(perms_array),
                            );
                        }
                ...
                json.insert(
                    ...
                    serde_json::Value::Array(db_perms_json),
                );
            }

            if let Some(create_table_default_permissions) =
                &data_lake_settings.create_table_default_permissions
            {
                ...
                json.insert(
                    ...
                    serde_json::Value::Array(table_perms_json),
                );
            }
            ...
            if let Some(trusted_resource_owners) = &data_lake_settings.trusted_resource_owners {
                ...
                json.insert(
                    ...
                    serde_json::Value::Array(owners_json),
                );
            }

            if let Some(allow_external_data_filtering) =
                data_lake_settings.allow_external_data_filtering
            {
                json.insert(
                    "AllowExternalDataFiltering".to_string(),
                    serde_json::Value::Bool(allow_external_data_filtering),
                );
            }

            if let Some(allow_full_table_external_data_access) =
                data_lake_settings.allow_full_table_external_data_access
            {
                json.insert(
                    "AllowFullTableExternalDataAccess".to_string(),
                    serde_json::Value::Bool(allow_full_table_external_data_access),
                );
            }

            if let Some(external_data_filtering_allow_list) =
                &data_lake_settings.external_data_filtering_allow_list
            {
                ...
                json.insert(
                    ...
                    serde_json::Value::Array(allow_list_json),
                );
            }

            if let Some(authorized_session_tag_value_list) =
                &data_lake_settings.authorized_session_tag_value_list
            {
                ...
                json.insert(
                    ...
                    serde_json::Value::Array(tags_json),
                );
            }
        }
    ...
    fn permission_to_json(
        &self,
        permission: &lakeformation::types::PrincipalResourcePermissions,
    ) -> serde_json::Value {
        let mut json = serde_json::Map::new();
        ...
        if let Some(resource) = &permission.resource {
            ...
            if let Some(database) = &resource.database {
                ...
                if let Some(catalog_id) = &database.catalog_id {
                    db_json.insert(
                        "CatalogId".to_string(),
                        serde_json::Value::String(catalog_id.clone()),
                    );
                }
                db_json.insert(
                    "Name".to_string(),
            ...
            if let Some(table) = &resource.table {
                ...
                table_json.insert(
                    ...
                    serde_json::Value::String(table.database_name.clone()),
                );
        ...
        if let Some(permissions) = &permission.permissions {
            ...
            json.insert(
                "Permissions".to_string(),
                serde_json::Value::Array(perms_json),
            );
        }
        ...
        if let Some(permissions_with_grant_option) = &permission.permissions_with_grant_option {
            ...
            json.insert(
                ...
                serde_json::Value::Array(grant_perms_json),
            );
        }
