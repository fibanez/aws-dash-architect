...
impl MskService {
    ...
    fn cluster_to_json(&self, cluster: &kafka::types::Cluster) -> serde_json::Value {
        ...
        if let Some(cluster_arn) = &cluster.cluster_arn {
            json.insert("ClusterArn".to_string(), serde_json::Value::String(cluster_arn.clone()));
            json.insert("ResourceId".to_string(), serde_json::Value::String(cluster_arn.clone()));

            // Extract cluster name from ARN
            if let Some(cluster_name) = cluster_arn.split('/').nth(1) {
                json.insert("ClusterName".to_string(), serde_json::Value::String(cluster_name.to_string()));
                json.insert("Name".to_string(), serde_json::Value::String(cluster_name.to_string()));
            }
        }

        if let Some(cluster_type) = &cluster.cluster_type {
            json.insert("ClusterType".to_string(), serde_json::Value::String(cluster_type.as_str().to_string()));
        }
        ...
        if let Some(current_version) = &cluster.current_version {
            json.insert("CurrentVersion".to_string(), serde_json::Value::String(current_version.clone()));
        }
        ...
        if let Some(state_info) = &cluster.state_info {
            ...
            if let Some(message) = &state_info.message {
                state_info_json.insert("Message".to_string(), serde_json::Value::String(message.clone()));
            }
            json.insert("StateInfo".to_string(), serde_json::Value::Object(state_info_json));
        }

        // Set default name if not available
        if !json.contains_key("Name") {
            json.insert("Name".to_string(), serde_json::Value::String("unknown-cluster".to_string()));
        }
        ...
        if let Some(provisioned) = &cluster.provisioned {
            ...
            if let Some(broker_node_group_info) = &provisioned.broker_node_group_info {
                ...
                if let Some(instance_type) = &broker_node_group_info.instance_type {
                    broker_json.insert("InstanceType".to_string(), serde_json::Value::String(instance_type.clone()));
                }
                ...
                if let Some(client_subnets) = &broker_node_group_info.client_subnets {
                    let subnets_json: Vec<serde_json::Value> = client_subnets
                        ...
                        .collect();
                    broker_json.insert("ClientSubnets".to_string(), serde_json::Value::Array(subnets_json));
                }

                provisioned_json.insert("BrokerNodeGroupInfo".to_string(), serde_json::Value::Object(broker_json));
            }
            ...
            if let Some(number_of_broker_nodes) = provisioned.number_of_broker_nodes {
                provisioned_json.insert("NumberOfBrokerNodes".to_string(), serde_json::Value::Number(serde_json::Number::from(number_of_broker_nodes)));
            }

            json.insert("Provisioned".to_string(), serde_json::Value::Object(provisioned_json));
        }
                    ...
                    .map(|config| {
                        ...
                        if let Some(subnet_ids) = &config.subnet_ids {
                            let subnets_json: Vec<serde_json::Value> = subnet_ids
                                ...
                                .map(|subnet| serde_json::Value::String(subnet.clone()))
                                .collect();
                            config_json.insert("SubnetIds".to_string(), serde_json::Value::Array(subnets_json));
                        }
                        if let Some(security_group_ids) = &config.security_group_ids {
                            let sgs_json: Vec<serde_json::Value> = security_group_ids
                                ...
                                .collect();
                            config_json.insert("SecurityGroupIds".to_string(), serde_json::Value::Array(sgs_json));
                        }
                        serde_json::Value::Object(config_json)
                    })
                    .collect();
                serverless_json.insert("VpcConfigs".to_string(), serde_json::Value::Array(configs_json));
            }

            json.insert("Serverless".to_string(), serde_json::Value::Object(serverless_json));
        }
