...
/// Normalizer for AWS Organizations Roots
pub struct OrganizationsRootNormalizer;

#[async_trait]
impl AsyncResourceNormalizer for OrganizationsRootNormalizer {
    async fn normalize(
        &self,
        ...
        if let Some(policy_types) = raw_response.get("PolicyTypes") {
            properties.insert("policy_types".to_string(), policy_types.clone());
        }
