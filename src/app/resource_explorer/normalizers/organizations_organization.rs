...
/// Normalizer for AWS Organizations Organization (singleton resource)
pub struct OrganizationsOrganizationNormalizer;

#[async_trait]
impl AsyncResourceNormalizer for OrganizationsOrganizationNormalizer {
    async fn normalize(
        &self,
        ...
        // Organization uses its ID as display name
        ...
        if let Some(feature_set) = raw_response.get("FeatureSet") {
            properties.insert("feature_set".to_string(), feature_set.clone());
        }

        if let Some(master_account_arn) = raw_response.get("MasterAccountArn") {
            properties.insert("master_account_arn".to_string(), master_account_arn.clone());
        }

        if let Some(master_account_id) = raw_response.get("MasterAccountId") {
            properties.insert("master_account_id".to_string(), master_account_id.clone());
        }
        ...
        let mut entry = ResourceEntry {
            ...
            is_child_resource: false, // Organization is a top-level resource
            account_color,
            region_color,
            query_timestamp,
        };
