...
impl GuardValidator {
    ...
    pub async fn new(compliance_programs: Vec<ComplianceProgram>) -> Result<Self> {
        ...
        for program in &compliance_programs {
            ...
            let should_update = registry.check_for_updates(program.clone()).await.unwrap_or(false);
            
            let rules = if should_update {
                // Download fresh rules
                log::info!("Downloading updated rules for compliance program: {:?}", program);
                registry.download_compliance_rules(program.clone()).await
                    .unwrap_or_else(|e| {
                        log::warn!("Failed to download rules for {:?}: {}. Will try cached version.", program, e);
                        HashMap::new()
                    })
            } else {
                // Use cached rules
        ...
        log::info!("GuardValidator initialized with {} rules from {} compliance programs", 
                   all_rules.len(), compliance_programs.len());
    ...
    pub async fn update_rules(&mut self) -> Result<()> {
        ...
        for program in &self.compliance_programs {
            match self.registry.download_compliance_rules(program.clone()).await {
                Ok(rules) => {
                    log::info!("Updated {} rules for compliance program: {:?}", rules.len(), program);
                    for (rule_name, rule_content) in rules {
                        all_rules.insert(rule_name, rule_content);
                    }
                }
        ...
        if !all_rules.is_empty() {
            ...
            log::info!("Guard rules updated successfully. Total rules: {}", self.rules.len());
        }
    ...
    pub async fn validate_template(&mut self, template: &CloudFormationTemplate) -> Result<GuardValidation> {
        ...
        for (rule_name, rule_content) in &self.rules {
            match self.validate_against_rule(&template_yaml, rule_name, rule_content).await {
                Ok(mut violations) => {
        ...
        log::debug!("Guard validation completed in {:?} with {} violations", duration, validation_result.violations.len());
    ...
    fn cache_validation_result(&mut self, template_hash: String, result: GuardValidation) {
        ...
        log::debug!("Cached validation result. Cache size: {}", self.validation_cache.len());
    }
    ...
    async fn validate_against_rule(
        ...
        self.validate_rule_directly(template_yaml, rule_name, rule_content).await
    }
    ...
    async fn validate_rule_directly(
        ...
        match self.validate_with_guard_engine(template_yaml, rule_content).await {
            Ok(violations) => {
                ...
                let violations: Vec<GuardViolation> = violations.into_iter().map(|msg| GuardViolation {
            ...
            Err(_) => {
                ...
                let violations = self.validate_guard_rule_pattern(
                    &template_value, 
                    rule_name, 
                    rule_content
                )?;
    ...
    async fn validate_with_guard_engine(&self, template_content: &str, rule_content: &str) -> Result<Vec<String>> {
            ...
            if let Some(resources) = template.get("Resources").and_then(|r| r.as_object()) {
                for (resource_name, resource) in resources {
                    if let Some(resource_type) = resource.get("Type").and_then(|t| t.as_str()) {
                        ...
                        violations.extend(self.check_guard_patterns(resource_name, resource_type, resource, rule_content));
                    }
                }
            }
        }
    ...
    fn check_guard_patterns(&self, resource_name: &str, resource_type: &str, resource: &serde_json::Value, rule_content: &str) -> Vec<String> {
        let mut violations = Vec::new();
            ...
            if rule_content.contains("PublicReadPolicy") && rule_content.contains("!= true") {
                if let Some(props) = resource.get("Properties") {
                    if props.get("PublicReadPolicy").and_then(|v| v.as_bool()) == Some(true) {
                        violations.push(format!("S3 bucket '{}' has public read policy enabled", resource_name));
                    }
                }
            }
            ...
            if rule_content.contains("BucketPolicy") && rule_content.contains("exists") {
                if let Some(props) = resource.get("Properties") {
                    if props.get("BucketPolicy").is_none() {
                        violations.push(format!("S3 bucket '{}' is missing required bucket policy", resource_name));
                    }
                }
            }
        }
            ...
            if rule_content.contains("StorageEncrypted") && rule_content.contains("== true") {
                if let Some(props) = resource.get("Properties") {
                    if props.get("StorageEncrypted").and_then(|v| v.as_bool()) != Some(true) {
                        violations.push(format!("RDS instance '{}' does not have storage encryption enabled", resource_name));
                    }
                }
            }
        }
        ...
        if resource_type == "AWS::EC2::SecurityGroup" {
            if rule_content.contains("SecurityGroupIngress") {
                if let Some(props) = resource.get("Properties") {
                    if let Some(ingress_rules) = props.get("SecurityGroupIngress").and_then(|v| v.as_array()) {
            ...
            for (resource_name, resource) in resources {
                if let Some(resource_type) = resource.get("Type").and_then(|t| t.as_str()) {
                    ...
                    if pattern.resource_type.is_empty() || pattern.resource_type == resource_type {
                        ...
                        if let Some(violation) = self.check_rule_conditions(
                            &pattern,
                            resource_name,
                            resource,
                            rule_name,
        ...
        for check in &pattern.property_checks {
            ...
            if violates_rule {
                return Some(GuardViolation {
                    ...
                    message: format!("Resource violates rule: {}", self.get_violation_message(rule_name, &check.property_path)),
                    severity: self.determine_severity(rule_name),
                    exempted: false,
                    exemption_reason: None,
                });
    ...
    fn get_nested_property<'a>(&self, value: &'a serde_json::Value, path: &str) -> Option<&'a serde_json::Value> {
    ...
    fn get_violation_message(&self, rule_name: &str, property_path: &str) -> String {
        match rule_name {
            ...
            name if name.contains("ENCRYPTION_ENABLED") => {
                format!("Resource should have encryption enabled ({})", property_path)
            }
            _ => {
                format!("Resource property '{}' violates compliance rule", property_path)
            }
        }
    }
    ...
    fn determine_severity(&self, rule_name: &str) -> ViolationSeverity {
        ...
        if rule_lower.contains("ssl") || rule_lower.contains("encryption") || rule_lower.contains("public") {
                                ...
                                for rule in rules_array {
                                    if let Some(rule_str) = rule.as_str() {
                                        if rule_str == rule_name {
                                            return (true, Some("Suppressed via guard.SuppressedRules".to_string()));
                                        }
                                    }
                                }
                            }
                                    ...
                                    if let Some(id) = rule_obj.get("id") {
                                        if let Some(id_str) = id.as_str() {
                                            ...
                                            if self.cfn_nag_rule_matches(id_str, rule_name) {
                                                let reason = rule_obj.get("reason")
                                                    .and_then(|r| r.as_str())
                                                    .map(|s| s.to_string())
                                                    .unwrap_or_else(|| "Suppressed via cfn_nag".to_string());
    ...
    fn process_violations_with_exemptions(
        ...
        violations.into_iter().map(|mut violation| {
            ...
            if exempted {
                log::debug!("Violation exempted: {} on {} - {:?}", 
                           violation.rule_name, 
                           violation.resource_name,
