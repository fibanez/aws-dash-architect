...
impl MessageInjector {
    ...
    pub fn queue_immediate(&mut self, injection_type: InjectionType) {
        self.queue(PendingInjection::new(injection_type, InjectionTrigger::Immediate));
    }
...
mod tests {
    ...
    fn test_injection_type_labels() {
        assert_eq!(InjectionType::SystemContext("test".into()).label(), "SystemContext");
        assert_eq!(
            InjectionType::ToolFollowUp {
                tool_name: "test".into(),
                context: "ctx".into()
        ...
        assert_eq!(InjectionType::MemorySummary("test".into()).label(), "MemorySummary");
        assert_eq!(InjectionType::Correction("test".into()).label(), "Correction");
    }
