...
impl AgentLogger {
    ...
    fn get_log_path(agent_id: AgentId) -> Result<PathBuf, std::io::Error> {
        // Generate timestamp prefix: YYYYMMDDHHmm format
        let timestamp = Utc::now().format("%Y%m%d%H%M").to_string();
        let filename = format!("{}-Agent-{}.log", timestamp, agent_id);
    ...
    pub fn get_debug_log_path(agent_id: AgentId) -> Result<PathBuf, std::io::Error> {
        // Generate timestamp prefix: YYYYMMDDHHmm format
        let timestamp = Utc::now().format("%Y%m%d%H%M").to_string();
        let filename = format!("{}-Agent-{}-debug.log", timestamp, agent_id);
