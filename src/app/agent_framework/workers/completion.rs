...
pub fn send_worker_completion(completion: WorkerCompletion) {
    stood::perf_checkpoint!(
        "awsdash.worker_completion.send.start",
        &format!("worker_id={}", completion.worker_id)
    );
    ...
    if let Some((result_slot, condvar)) = registry.get_mut(&worker_id) {
        ...
        *result_slot = Some(completion);
        ...
        stood::perf_checkpoint!(
            "awsdash.worker_completion.send.notified",
            &format!(
                "worker_id={}, success={}, execution_time_ms={}",
                worker_id, is_success, execution_time_ms
            )
        );
        tracing::info!(
        ...
        stood::perf_checkpoint!(
            "awsdash.worker_completion.send.no_waiter",
...
pub fn wait_for_worker_completion(worker_id: AgentId, timeout: Duration) -> Result<String, String> {
    stood::perf_checkpoint!(
        "awsdash.worker_completion.wait.start",
        &format!(
    ...
    let _wait_guard = stood::perf_guard!(
        "awsdash.worker_completion.wait",
        &format!("worker_id={}", worker_id)
    );
    ...
    stood::perf_checkpoint!(
        "awsdash.worker_completion.wait.condvar_wait.start",
        &format!("worker_id={}", worker_id)
    );
    let result = {
        ...
        if timeout_result.timed_out() {
            stood::perf_checkpoint!(
                "awsdash.worker_completion.wait.timeout",
                &format!("worker_id={}", worker_id)
            );
        ...
        stood::perf_checkpoint!(
            "awsdash.worker_completion.wait.condvar_wait.notified",
            &format!("worker_id={}", worker_id)
        );
    ...
    stood::perf_checkpoint!(
        "awsdash.worker_completion.wait.complete",
        &format!(
            "worker_id={}, success={}, execution_time_ms={}",
            worker_id,
            result.result.is_ok(),
