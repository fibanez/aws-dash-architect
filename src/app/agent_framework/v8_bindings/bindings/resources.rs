...
async fn execute_complete_query(
        ...
        let resources_to_enrich: Vec<ResourceEntry> = phase1_resources
            ...
            .filter(|r| {
                enrichable_types.contains(&r.resource_type.as_str())
                    && r.detailed_timestamp.is_none() // Only enrich if not already enriched
            })
            .cloned()
            .collect();
    ...
    info!(
        ...
        filtered_resources.iter().filter(|r| r.detailed_timestamp.is_some()).count()
    );
...
async fn get_resource_schema_internal(resource_type: &str) -> Result<GetResourceSchemaResult> {
    ...
    if let Some(example) = example_resource {
        ...
        // Order: properties -> properties -> detailed_properties (later overwrites earlier)
        ...
        // Layer 2: properties (Phase 1 List API response)
        if let Some(raw) = example.properties.as_object() {
            for (key, value) in raw {
                merged_properties.insert(key.clone(), value.clone());
            }
        }

        // Note: detailed_properties are now merged directly into properties during Phase 2
        // So properties already contains merged data if Phase 2 completed
...
pub fn execute_query_cached_resources(args: QueryCachedResourcesArgs) -> Result<QueryCachedResourcesResult> {
    tokio::task::block_in_place(|| {
        tokio::runtime::Handle::current()
...
async fn query_cached_resources_internal(
    ...
    for entry in &all_resources {
        ...
        // Layer 2: properties (Phase 1 List API data)
        ...
        // Layer 2: properties
        if let Some(raw) = entry.properties.as_object() {
            for (key, value) in raw {
                merged_properties.insert(key.clone(), value.clone());
            }
        }
