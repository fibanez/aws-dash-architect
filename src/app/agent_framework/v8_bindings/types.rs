...
pub fn to_v8_value<'a, T: Serialize>(
    ...
    let json_str =
        serde_json::to_string(value).map_err(|e| anyhow!("Failed to serialize to JSON: {}", e))?;

    // Create V8 string from JSON
    let v8_str =
        v8::String::new(scope, &json_str).ok_or_else(|| anyhow!("Failed to create V8 string"))?;

    // Parse JSON in V8 context to create JavaScript value
    let json =
        v8::json::parse(scope, v8_str).ok_or_else(|| anyhow!("Failed to parse JSON in V8"))?;
...
pub fn from_v8_value<T: for<'de> Deserialize<'de>>(
    ...
    let json_str =
        v8::json::stringify(scope, value).ok_or_else(|| anyhow!("Failed to stringify V8 value"))?;
