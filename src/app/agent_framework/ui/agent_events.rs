...
use crate::app::agent_framework::core::types::AgentId;
...
pub enum AgentUIEvent {
    ...
    ///
    /// Sent when a TaskWorker or ToolBuilderWorker is created by a TaskManager.
    ...
    WorkerToolCompleted {
        ...
        tool_name: String,
        success: bool,
        /// Tokens used for this specific tool call (if available)
    ...
    ///
    /// Sent from Pages Manager webview when user clicks Edit on a page.
    /// The UI will create a TaskManager agent with the page workspace set
    /// and show a message prompting the user to describe changes.
    OpenPageForEdit {
        /// The page workspace name (folder name in ~/.local/share/awsdash/pages/)
        page_name: String,
    },
}
...
impl AgentUIEvent {
    ...
    pub fn worker_completed(worker_id: AgentId, parent_id: AgentId, success: bool) -> Self {
    ...
    /// Create a new OpenPageForEdit event
    pub fn open_page_for_edit(page_name: String) -> Self {
        Self::OpenPageForEdit { page_name }
    }
}
...
mod tests {
    ...
    fn test_worker_progress_events() {
        ...
        // Test WorkerStarted
        let started =
            AgentUIEvent::worker_started(worker_id, parent_id, "Listing instances".to_string(), 5);
        assert!(matches!(
            started,
        ...
        let worker_completed = AgentUIEvent::worker_completed(worker_id, parent_id, true);
        assert!(matches!(
            worker_completed,
