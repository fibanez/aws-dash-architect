...
impl ProcessingAnimation {
    ...
    fn render_orbital_dots(&self, ui: &mut Ui, center: Pos2) {
        ...
        // Pulsing radius effect: oscillate between 60% and 100% of orbit radius
        let pulse_phase = phase * 1.5; // Pulse slightly faster than rotation
        let pulse = (pulse_phase.sin() + 1.0) / 2.0; // 0.0 to 1.0
        let min_radius = Self::ORBIT_RADIUS * 0.6;
        let max_radius = Self::ORBIT_RADIUS;
        let current_radius = min_radius + (max_radius - min_radius) * pulse;

        for i in 0..Self::NUM_DOTS {
            let angle = phase + (i as f32 * TAU / Self::NUM_DOTS as f32);
            let offset = Vec2::angled(angle) * current_radius;
    ...
    fn render_wave_bars(&self, ui: &mut Ui, center: Pos2) {
        ...
        let total_width = Self::NUM_BARS as f32 * Self::BAR_WIDTH
