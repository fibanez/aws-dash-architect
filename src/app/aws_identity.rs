...
impl AwsIdentityCenter {
    ...
    fn identify_sso_management_account(&mut self) -> Result<(), String> {
        ...
        if let Some(credentials) = &self.default_role_credentials {
            ...
            log::info!(
                "⏱️ [AWS] Runtime creation (identify_sso_management_account) took {:?}",
                rt_start.elapsed()
            );
    ...
    pub fn start_device_authorization(&mut self) -> Result<DeviceAuthorizationData, String> {
        ...
        log::info!(
            "⏱️ [AWS] Runtime creation (start_device_authorization) took {:?}",
            rt_start.elapsed()
        );
    ...
    pub fn complete_device_authorization(&mut self) -> Result<(), String> {
        ...
        thread::spawn(move || {
            ...
            let runtime = match Runtime::new() {
                Ok(rt) => {
                    log::info!(
                        "⏱️ [AWS] Runtime creation (poll_for_token_bg_thread) took {:?}",
                        rt_start.elapsed()
                    );
                    rt
                }
    ...
    pub fn get_account_credentials(
        ...
        let runtime =
            Runtime::new().map_err(|e| format!("Failed to create Tokio runtime: {}", e))?;
        log::info!(
            "⏱️ [AWS] Runtime creation (get_account_role_credentials) took {:?}",
            rt_start.elapsed()
        );
    ...
    fn discover_aws_reserved_sso_role(&mut self) -> Result<String, String> {
        ...
        let runtime =
            Runtime::new().map_err(|e| format!("Failed to create Tokio runtime: {}", e))?;
        log::info!(
            "⏱️ [AWS] Runtime creation (discover_aws_reserved_sso_role) took {:?}",
            rt_start.elapsed()
        );
    ...
    pub fn extract_infrastructure_info(
        ...
        let runtime =
            Runtime::new().map_err(|e| format!("Failed to create Tokio runtime: {}", e))?;
        log::info!(
            "⏱️ [AWS] Runtime creation (extract_infrastructure_info) took {:?}",
            rt_start.elapsed()
        );
