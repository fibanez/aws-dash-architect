...
use awsdash::app::projects::{Project, Environment, AwsRegion, AwsAccount};
use serde_json;
...
fn test_project_set_compliance_programs() {
    ...
    assert!(project.compliance_programs.contains(&ComplianceProgram::NIST80053R5));
    assert!(project.compliance_programs.contains(&ComplianceProgram::PCIDSS));
    assert!(project.compliance_programs.contains(&ComplianceProgram::HIPAA));
}
...
fn test_environment_specific_compliance() {
    ...
    env_compliance.insert(
        "Dev".to_string(),
        vec![ComplianceProgram::NIST80053R5],
    );
    env_compliance.insert(
        "Prod".to_string(),
        vec![ComplianceProgram::NIST80053R5, ComplianceProgram::PCIDSS, ComplianceProgram::FedRAMP],
    );
...
fn test_custom_guard_rules() {
    ...
    assert!(project.custom_guard_rules.contains(&"/path/to/custom/company-policy.guard".to_string()));
}
...
fn test_project_deserialization_with_guard() {
    ...
    let has_custom = project.compliance_programs.iter().any(|p| {
        matches!(p, ComplianceProgram::Custom(name) if name == "CompanyPolicy")
    });
    assert!(has_custom);
}
...
fn test_project_file_persistence() -> Result<()> {
    ...
    assert!(loaded_project.compliance_programs.contains(&ComplianceProgram::HIPAA));
    assert_eq!(loaded_project.custom_guard_rules.len(), 1);
    assert_eq!(loaded_project.custom_guard_rules[0], "/custom/hipaa-extra.guard");
