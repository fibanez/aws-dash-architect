use awsdash::app::cfn_guard::{GuardValidator, ComplianceProgram};
...
async fn test_guard_template_integration() {
    ...
    let validator = GuardValidator::new(vec![ComplianceProgram::NIST80053R5]).await.unwrap();
...
async fn test_guard_template_with_violations() {
    ...
    assert_eq!(validation.violations[0].rule_name, "S3_BUCKET_PUBLIC_READ_PROHIBITED");
}