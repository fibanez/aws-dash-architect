use awsdash::app::compliance_discovery::{
    ComplianceDiscovery, AvailableComplianceProgram, ComplianceProgramCache, 
...
async fn test_discover_available_programs() {
    ...
    assert!(program_names.iter().any(|name| name.contains("nist") || name.contains("NIST")));
}
...
async fn test_parse_repository_structure() {
    ...
    let programs = discovery.parse_repository_structure(mock_structure).await.unwrap();
...
async fn test_github_api_error_handling() {
    ...
    match result {
        ...
        Err(err) => {
            ...
            assert!(error_msg.contains("GitHub") || error_msg.contains("network") || error_msg.contains("API"));
        }
    }
}
...
async fn test_program_search_filtering() {
    ...
    for program in &nist_programs {
        assert!(program.name.to_lowercase().contains("nist") || 
