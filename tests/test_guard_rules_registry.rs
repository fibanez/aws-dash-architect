...
use awsdash::app::guard_rules_registry::{
    ComplianceProgram, GuardRulesRegistry, RuleMetadata, RuleVersion,
};
...
async fn test_download_compliance_rules() {
    ...
    let result = registry
        .download_compliance_rules(ComplianceProgram::NIST80053R5)
        .await;
...
async fn test_rules_caching() {
    ...
    let rules2 = registry
        .get_cached_rules(ComplianceProgram::NIST80053R5)
        .await
        .unwrap();
...
async fn test_rule_versioning() {
    ...
    let metadata = registry
        .get_rule_metadata(ComplianceProgram::NIST80053R5)
        .await;
...
async fn test_offline_mode() {
    ...
    let _rules = registry
        .download_compliance_rules(ComplianceProgram::PCIDSS)
        .await
        .unwrap();
    ...
    let cached_rules = offline_registry
        .get_cached_rules(ComplianceProgram::PCIDSS)
        .await;
...
async fn test_rule_update_detection() {
    ...
    let has_updates = registry
        .check_for_updates(ComplianceProgram::NIST80053R5)
        .await;
...
async fn test_cache_cleanup() {
    ...
    // Clean cache
    let result = registry
        .clear_cache(Some(ComplianceProgram::NIST80053R5))
        .await;
    assert!(result.is_ok());
