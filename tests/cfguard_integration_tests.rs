...
use awsdash::app::cfn_guard::{GuardValidator, ComplianceProgram, ViolationSeverity};
...
async fn test_real_guard_rules_download() -> Result<()> {
    ...
    for program in programs_to_test {
        ...
        assert!(download_result.len() > 0, 
                "Should have downloaded rules for {:?}", program);
        ...
        for (rule_name, rule_content) in &download_result {
            ...
            assert!(has_guard_keywords, 
                   "Rule {} should contain Guard DSL keywords. Content: {}", 
                   rule_name, rule_content);
        }
        
        println!("✓ Downloaded {} rules for {:?}", download_result.len(), program);
    }
...
async fn test_real_template_compliance_validation() -> Result<()> {
    ...
    assert!(validation_result.total_rules > 0, "Should have evaluated some rules");
    ...
    println!("  - Total rules evaluated: {}", compliant_validation.total_rules);
    println!("  - Violations found: {}", compliant_validation.violations.len());
    
    // Should have fewer violations than the non-compliant template
    assert!(compliant_validation.violations.len() <= validation_result.violations.len(),
...
async fn test_metadata_exemption_detection() -> Result<()> {
    ...
    println!("  - Total violations: {}", validation_result.violations.len());
    
    // Check if exemptions are properly detected
    for violation in &validation_result.violations {
        println!("  - Violation: {} on {}", violation.rule_name, violation.resource_name);
...
async fn test_violation_severity_classification() -> Result<()> {
    ...
    assert!(validation_result.violations.len() > 0, "Should have found some violations");
...
async fn test_validation_caching_performance() -> Result<()> {
    ...
    assert_eq!(first_result.violations.len(), second_result.violations.len());
    ...
    println!("Cache speedup ratio: {:.2}x", 
    ...
    for program in programs_to_test {
        ...
        match metadata_result {
            Ok(metadata) => {
                ...
                assert!(!metadata.last_updated.is_empty(), "Last updated should not be empty");
                assert!(metadata.rules_count > 0, "Should have some rules");
                assert!(metadata.source_url.contains("aws"), "Should reference AWS source");
            }
            Err(e) => {
...
async fn test_error_handling() -> Result<()> {
    ...
    assert!(template_result.is_err(), "Should fail to parse invalid JSON");
    ...
    assert!(validation_result.compliant, "Empty template should be compliant");
    assert_eq!(validation_result.violations.len(), 0, "Empty template should have no violations");
