...
use awsdash::app::bulk_rule_downloader::{
    BulkRuleDownloader, ComplianceRuleSet, DownloadProgress, DownloadStatus, GuardRuleFile,
    RuleDownloadManager, RuleIndex, RuleStorage,
};
...
async fn test_download_progress_tracking() {
    ...
    let result = downloader
        .download_with_progress(&program, progress.clone())
        .await;
...
async fn test_rule_indexing() {
    ...
    // Test lookups
    let s3_rules = rule_index
        .find_rules_by_resource_type("AWS::S3::Bucket")
        .await
        .unwrap();
    assert_eq!(s3_rules.len(), 1);
    assert!(s3_rules.contains(&"s3_bucket_rule".to_string()));

    let program_rules = rule_index
        .find_rules_by_program("indexed_program")
        .await
        .unwrap();
